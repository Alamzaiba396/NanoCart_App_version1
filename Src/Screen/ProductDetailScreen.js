...
const ProductDetail = ({navigation}) => {
      ...
      <ScrollView
        ...
        <View style={styles.colors}>
          {['green', 'red', 'black', 'navy', 'gold'].map((color, idx) => (
            <TouchableOpacity key={idx} style={[styles.colorBox, { backgroundColor: color }]} />
          ))}
        </View>

        {/* Product Info */}
        <View style={styles.details}>
          <Text style={styles.title}>MAAHI Popular: Georgette Saree in Red</Text>
          ...
          <Text style={styles.delivery}>Fastest 2–3 days delivery to 100+ pincodes</Text>
        </View>
    ...
    {['XS', 'S', 'M', 'L', 'XL', '2XL', '3XL'].map((size, idx) => (
      <TouchableOpacity key={idx} style={styles.sizeBox}>
        ...
        <AccordionItem title="About the Product">
          ...
          <Text>
            Crew neck with an attached hood, Adjustable hood with drawcord, A Oversized fit with long sleeves, A spacious kangaroo pocket on the front
          </Text>
          <Text>Material: 60% Cotton 40% Poly | 280 GSM Fleece</Text>
          <Text>Care: Machine wash according to instructions on the care label.</Text>
          <Text style={{ fontStyle: 'italic' }}>
            Style Note: You can’t go wrong with a classic hoodie! Dress it up with a pair of denims or dress it down with a pair of joggers.
          </Text>
        </AccordionItem>

        <AccordionItem title="Check Delivery at Your Pincode">
          <Text>Enter your pincode to check delivery options and estimated time.</Text>
        </AccordionItem>

        <AccordionItem title="Return Policies">
          <Text>Easy return within 7 days of delivery. Product must be unused and with all tags intact.</Text>
        </AccordionItem>
        ...
        <AccordionItem title="Ratings & Reviews">
          <View style={styles.ratingContainer}>
            <View style={styles.ratingRow}>
              ...
              {[...Array(4)].map((_, index) => (
                <Icon key={index} name="star" size={16} color="#F57C00" style={styles.starIcon} />
              ))}
              ...
              <View style={styles.reviewHeader}>
                <View style={styles.ratingRow}>
                  ...
                  {[...Array(Math.floor(review.rating))].map((_, i) => (
                    <Icon key={i} name="star" size={14} color="#F57C00" style={styles.starIcon} />
                  ))}
                  {review.rating % 1 !== 0 && (
                ...
                <Text style={styles.reviewSize}>Size bought: {review.size}</Text>
              </View>
          ...
          >
            {suggestionImages.map((image, index) => (
              <View key={index} style={styles.suggestionCard}>
                <Image source={image} style={styles.suggestionImage} resizeMode="cover" />
  ...
  <TouchableOpacity  onPress={() => navigation.navigate('Cart')} style={styles.addToCartButton}>
